...
#' @param day end_date A POSIXt, Date or string that can be coerced into Date by \code{\link{as_date}} indicating the day for witch to create the data table.
#'
#' @export
#'
...
make_two_users_dt <- function(user1_data, user2_data, day) {
  ...
  user2_data <- user2_data[as_date(day) <= start_time 
                           ...
                           .(start_time, end_time)
                                    ...
                                    end_time = ceiling_date(end_time, "minutes"),
                                                            user = 2)]
  data <- rbindlist(list(user1_data, user2_data))
}
