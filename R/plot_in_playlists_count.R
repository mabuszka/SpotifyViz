...
#' Either counting the number of tracks played or 
...
#' @param time A logical scalar indicating whether to count number of tracks played or the time they were played.
#' @param time_unit A character vector indicating the time unit to use if showing an absolute value of time played.
#' Permitted vales are "second", "minute", "hour"
#'
#' @import data.table
#' @import ggplot2
#' 
...
plot_in_playlists_count <- function(str_his_with_playlist_wide, as_percentage = FALSE, time = FALSE, time_unit = "minute") {
  played <- in_any <- s_played <- NULL
  
  if (time) {
    time_units <- c("second" = 1, "minute" = 60, "hour" = 3600)
    time_dt  <- str_his_with_playlist_wide[, .(played = sum(s_played) / time_units[[time_unit]]) , by = in_any]
    y_label  <- paste("Time played (in ", time_unit,"s)", sep = "")
  ...
  if (!as_percentage) {
    vis <- vis + geom_bar(aes(x = in_any, y = played ), stat = "identity")+
            labs(y = y_label)
  }
  
  else {
    vis <- vis + 
            geom_bar(aes(x = in_any, y = played * 100 / sum(played) ), stat = "identity") +
