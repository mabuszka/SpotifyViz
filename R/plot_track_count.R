...
#' @param only_skipped A logical scalar indicating whether to show 
#' all played tracks or only those that were skipped.
#' @param by A character vector indicating for which periods 
#' to split the time for counting tracks played/skipped.
...
plot_track_count <-
  function(streaming_history,
    ...
    pretty_breaks <-
      as.Date(lubridate::pretty_dates((streaming_history$end_time), 5))
    ...
    viz <- switch(
      ...
      "month" = viz + scale_x_date(date_labels = "%m.%y", date_breaks = "2 months"),
