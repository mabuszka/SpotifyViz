...
#' @param start_date start_date A POSIXt,Date or string that can be coerced 
#' into Date by \code{\link{as_date}} indicating start of the period of time.
#' @param end_date end_date A POSIXt, Date or string that can be coerced 
#' into Date by \code{\link{as_date}} indicating end of the period of time.
#'
#' @return A streaming history data table containing only entries between 
...
filter_streaming_history <- function(streaming_history, start_date, end_date) {
  ...
  streaming_history[as_date(start_date) <= start_time &
