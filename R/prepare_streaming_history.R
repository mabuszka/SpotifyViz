...
#'
#' Prepares the data table containing streaming history for further operations.
#'  Changes the names of the base columns to: end_time, artist_name, track_name, s_played. 
#'  Changes the end_time 
#'  column from character to POSIXt date, s_played to a duration in seconds. Adds new columns: 
#'  start_time - estimated start time calculated from end_time and s_played, skipped - 
#'  TRUE if the track was 
...
#' 
#' @return A data table containing streaming history from spotify suited for being used 
#' in rest of the functions from the package.
#'
#' @export
#' 
#' @import data.table
...
prepare_streaming_history <- function(streaming_history){
  ...
  end_time <- s_played <- weekday <- start_time <- NULL
