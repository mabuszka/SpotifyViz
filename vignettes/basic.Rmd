...
This vignette introduces to basic functionalites of spoitfyviz package. 
...
In futher examples data provided by the package will be used.

```{r}
streaming_history <- spotifyviz::CleanStreamHis
  
search_queries <- spotifyviz::CleanSearchQueries
  
playlists <- spotifyviz::CleanPlaylist
...
## str_his_with_playlists_wide, srt_his_with_playlist_long
This functions make the data table containing information about user streaming history with addiotional information on whether the track was part of one of the playlist provided by the playist data table. The wide version gives the output in the wide form, and the long version in the long form.
...
This function adds the "listening number" to the streaming history data table - the number which determines to which listening session (the session - the tracks were played with less than `mins` minutes of breaks beetween them) the track belongs.
...
This function makes the table with brief summary of the data filtered to the period of time provided. It uses `how_long_listened` and `how_many_skipped` functions.
...
This function finds the most played tracks or artist and returs a data table with either track or artist name and the times it was played.
...
# for_view = TRUE : show results in format good for reading but not the best for futher computations 
# 
how_long_listened(streaming_history, start_date = "2018-01-01", end_date = "2019-01-01", as_percentage = TRUE, for_viewing = TRUE)
```


## how_many_skipped

This function counts how many tracks were skipped (their play time was less than 10s) in given time period. Either as a number or as a perentage of all tracks played in given time period.
...
how_many_skipped(streaming_history, start_date = "2018-01-01", end_date = "2019-01-01", as_percentage = TRUE)
```
...
This function plots either daily or hourly habbits of listening of the user.
